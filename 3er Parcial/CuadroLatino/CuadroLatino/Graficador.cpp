...
void Graficador::dibujarCuadroLatino(const std::vector<std::vector<int>>& matriz) {
    try {
        ...
        initwindow(width, height, "Triángulo Mágico");
        ...
        setcolor(BLACK);
        outtextxy(width / 2, 20, (char*)"Triángulo Mágico");
        ...
        for (int i = 0; i < n; i++) {
            for (int j = 0; j <= i; j++) {
                // Dibujar solo los números en los bordes del triángulo
                if (j == 0 || j == i || i == n - 1) {
        ...
        throw std::runtime_error("Error al dibujar el triángulo mágico: " + std::string(e.what()));
    }
}
